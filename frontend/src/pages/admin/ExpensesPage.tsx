    ...
    return <>
        <div className="content">
            <div className="mb-9">
                ...
                <h5 className="mb-4">Total Expenses Amount  : {loading ? "calculating ...." : expenses.length === 0 ? <span>₦0</span> : <span>₦{(expenses.reduce((a, c) => a + c.price!, 0)).toLocaleString()}</span>}   </h5>
                <div className="table-responsive scrollbar ms-n1 ps-1">
                    {
                        loading ? <h4 className="m-3">Loading Expenses ....</h4> : error ? <div className="alert alert-danger p-2 mb-3 text-light">{error}</div> : expenses.length === 0 ? <div className="alert alert-primary p-2 mb-3">No Expenses Added Yet</div> :
                            ...
                            <table className="table table-sm fs--1 mb-0">
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th className="sort align-middle" scope="col" data-sort="customer" style={{ width: '15%', minWidth: 200 }}>DESCRIPTION</th>
                                ...
                                <tbody className="list" id="members-table-body">
                                    {expenses.map(expense => {
                                        return <tr key={expense._id} className="hover-actions-trigger btn-reveal-trigger position-static">
                                            ...
                                            <td className="email align-middle white-space-nowrap">{expense.price && expense.price.toLocaleString()}</td>
                                            ...
                                            {userInfo && userInfo.isAdmin &&
                                                <td className="last_active align-middle text-end white-space-nowrap text-700">{expense.branch}</td>
                                            }
                                            <td className="last_active align-middle text-end white-space-nowrap text-700">{expense.createdAt && (new Date(expense.createdAt)).toString().substr(0, 11)}</td>
                                            <td className="joined align-middle white-space-nowrap text-700 text-end">{format(new Date(expense.createdAt), "hh : mm a")}</td>
                                            {userInfo && userInfo.isAdmin &&
