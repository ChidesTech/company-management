    ...
    return <>
        <div className="content">
            <div className="mb-9">
                ...
                <h5 className="mb-4">Total Amount Generated : {loading ? "calculating ...." : records.length === 0 ? <span>₦0</span> : <span>₦{(records.reduce((a, c) => a + c.price!, 0)).toLocaleString()}</span>}   </h5>
                            ...
                            <table className="table table-sm fs--1 mb-0">
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th className="sort align-middle" scope="col" data-sort="customer" style={{ width: '15%', minWidth: 200 }}>SERVICE</th>
                                        <th className="sort align-middle" scope="col" data-sort="email" style={{ width: '15%', minWidth: 200 }}>PRICE(₦)</th>
                                        <th className="sort align-middle pe-3" scope="col" data-sort="mobile_number" style={{ width: '20%', minWidth: 200 }}>RENDERER</th>
                                        <th className="sort align-middle" scope="col" data-sort="city" style={{ width: '10%' }}>RECORDER</th>
                                        {userInfo && userInfo.isAdmin &&
                                            <th className="sort align-middle text-end" scope="col" data-sort="last_active" style={{ width: '21%', minWidth: 200 }}>BRANCH</th>
                                        }
                                        <th className="sort align-middle text-end" scope="col" data-sort="last_active" style={{ width: '21%', minWidth: 200 }}>DATE</th>
                                        <th className="sort align-middle text-end pe-0" scope="col" data-sort="joined" style={{ width: '19%', minWidth: 200 }}>TIME</th>
                                        {userInfo && userInfo.isAdmin &&
                                            <th className="sort align-middle text-end pe-0" scope="col" data-sort="joined" style={{ width: '19%', minWidth: 200 }}>ACTIONS</th>
                                        }
                                    </tr>
                                </thead>
                                <tbody className="list" id="members-table-body">
                                    {records.map(record => {
                                        return <tr key={record._id} className="hover-actions-trigger btn-reveal-trigger position-static">
                                            ...
                                            <td className="email align-middle white-space-nowrap">{record.price && record.price.toLocaleString()}</td>
                                            <td className="mobile_number align-middle white-space-nowrap"> <a className="d-flex align-items-center text-900 text-hover-1000" href="#!">
                                            ...
                                            <td className="city align-middle white-space-nowrap text-900"> <a className="d-flex align-items-center text-900 text-hover-1000" href="#!">
                                            ...
                                            {userInfo && userInfo.isAdmin &&
                                                <td className="last_active align-middle text-end white-space-nowrap text-700">{record.branch}</td>
                                            }
                                            <td className="last_active align-middle text-end white-space-nowrap text-700">{record.createdAt && (new Date(record.createdAt)).toString().substr(0, 11)}</td>
                                            <td className="joined align-middle white-space-nowrap text-700 text-end">{format(new Date(record.createdAt), "hh : mm a")}</td>
                                            {userInfo && userInfo.isAdmin &&
